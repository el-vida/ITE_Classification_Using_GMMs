# In this program, we want to add useful information about 
# participants to a given dataframe (which has to contain a participant_id column)
...
INPUT_PATH  = r'C:\\Users\\eu_el\\Desktop\\VB_Schnittstelle\\Dataset\\Log' + str(LOG_ID) + '\\log_' + str(LOG_ID) + '_valid_processed_new.csv'
OUTPUT_PATH = r'C:\\Users\\eu_el\\Desktop\\VB_Schnittstelle\\Dataset\\Log' + str(LOG_ID) + '\\Participant_info\\log_' + str(LOG_ID) + '_valid_processed_new_with_participants_after_process_' 

# parser = argparse.ArgumentParser(description='Program adding columns with useful participant information to given dataframe.')
# parser.add_argument('--path',required=True,help='Path to the log data to be modified')
...
def add_participant_columns_to_(log, participants, process_id):
    ...
    for row in tqdm(range(log.shape[0])):
        index = log.index[row]
...
if(__name__ == "__main__"):
    ...
    for i in tqdm(range(num_processes)):
        ...
        p = Process(target=add_participant_columns_to_, args=[logs[i], log_participants, i])
        processes.append(p)
    # start processes
    for p in processes:
        p.start()
