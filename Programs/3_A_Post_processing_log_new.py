...
INPUT_PATH  = r'~\\Desktop\\VB_Schnittstelle\\Dataset\\Log' + str(LOG_ID) + '\\log_' + str(LOG_ID) + '_valid_processed_new_with_participants_and_user_groups.csv'
OUTPUT_PATH = r'~\\Desktop\\VB_Schnittstelle\\Dataset\\Log' + str(LOG_ID) + '\\Post_Processing_Decryption\\log_' + str(LOG_ID) + '_valid_post_processed_new_decrypted_after_process_'
...
def Apply_post_processing_steps(log_process, process_id):
...
if(__name__ == "__main__"):
    ...
    for i in range(num_processes):
        ...
        p = Process(target=Apply_post_processing_steps, args=[logs[i], i])
        processes.append(p)
    # start processes
