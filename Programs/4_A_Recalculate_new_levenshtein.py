...
INPUT_PATH  = r'C:\\Users\\eu_el\\Desktop\\VB_Schnittstelle\\Dataset\\Log' + str(LOG_ID) + '\\log_' + str(LOG_ID) + '_valid_post_processed_new_decrypted.csv'
OUTPUT_PATH = r'~\\Desktop\\VB_Schnittstelle\\Dataset\\Log' + str(LOG_ID) + '\\New_Levenshtein\\log_' + str(LOG_ID) + '_valid_post_processed_new_decrypted_new_levenshtein_after_process_'

def recalculate_levenshtein_distance(log_process, process_id):
    log_process['new_lev_dist'] = -7

    for row in tqdm(range(log_process.shape[0])):
        ## Very first row
...
if(__name__ == "__main__"):
    ...
    for i in range(num_processes):
        ...
        p = Process(target=recalculate_levenshtein_distance, args=[logs[i], i])
        processes.append(p)
    # start processes
    for p in processes:
        p.start()
