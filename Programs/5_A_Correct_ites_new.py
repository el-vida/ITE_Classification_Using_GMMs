...
INPUT_PATH  = r'C:\\Users\\eu_el\\Desktop\\VB_Schnittstelle\\Dataset\\Log' + str(LOG_ID) + '\\log_' + str(LOG_ID) + '_valid_post_processed_new_decrypted_new_levenshtein.csv'
OUTPUT_PATH = r'C:\\Users\\eu_el\\Desktop\\VB_Schnittstelle\\Dataset\\Log' + str(LOG_ID) + '\\Corrected\\log_' + str(LOG_ID) + '_valid_post_processed_new_decrypted_new_levenshtein_corrected_after_process_'

def infer_ite_no_swype(log_process, process_id):
    ...
    for row in tqdm(range(len(log_index)),position=1):
...
if(__name__ == "__main__"):
    ...
    for i in range(num_processes):
        ...
        p = Process(target=infer_ite_no_swype, args=[logs[i], i])
        processes.append(p)
    # start processes
    for p in processes:
        p.start()
    # let processes join
    for p in tqdm(processes, position=0):
        p.join()
