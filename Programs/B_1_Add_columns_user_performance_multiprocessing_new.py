...
def Prepare_datasets(log_comb_ite_with_participants_process, process_id, OUTPUT_PATH):
...
def Run_B_1(LOG_ID):
    ...
    INPUT_PATH  = r'~\\Desktop\\VB_Schnittstelle\\Dataset\\Data\\Log' + str(LOG_ID) + '\\log_' + str(LOG_ID) + '_valid_processed_new_with_participants.csv'
    OUTPUT_PATH = r'~\\Desktop\\VB_Schnittstelle\\Dataset\\Data\\Log' + str(LOG_ID) + '\\User_Performance\\log_' + str(LOG_ID) + '_valid_processed_new_with_user_groups_after_process_'
    ...
    for i in range(num_processes):
        ...
        p = Process(target=Prepare_datasets, args=[logs[i], i, OUTPUT_PATH])
        processes.append(p)
    # start processes
